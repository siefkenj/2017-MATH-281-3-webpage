...
\begin{document}
	...
	\begin{enumerate}
		\item For each of the following statements, produce a counterexample to show that the statement is {\bf false} in
			general.
			\begin{enumerate}
		...
		\item Let $\{\vec v_1,\vec v_2,\vec v_3\}\subset \R^3$ be a linearly independent set.
			Suppose a linear transformation $T:\R^3\to\R^3$ satisfies $T(\vec v_i) = \vec v_i$ for $i\in\{1,2,3\}$.
